---tạo database 
CREATE DATABASE NguyenDucCuong_2050531200110_join
go
USE NguyenDucCuong_2050531200110_join
go
...
GO
INSERT INTO SanPham
VALUES	('SP01',N'BÁNH TRÁNG','50000','500','30'),
		('SP02',N'BÚN TIÊU','60000','30','5'),
		('SP03',N'MÁY ẢNH','30000000','40','2'),
		('SP04',N'BÁNH MÌ','900000','1000','1'),
		('SP05',N'ĐẬU HỦ','500000','90','90'),
		('SP06',N'KẸO','100000','60','40'),
		('SP07',N'BÁNH XÈO','560000','20','0'),
		('SP08',N'BÁNH GẠO','320000','40','0'),
		('SP09',N'BÀN PHÍM CƠ','11000','60','50'),
		('SP10',N'MÀN HÌNH','2120000','240','60');
GO
INSERT INTO ChiTietPhieuNhap
VALUES	('PN1','SP01','20','10000'),
		('PN2','SP02','65','50000'),
		('PN3','SP03','56','30000'),
		('PN4','SP02','100','20000'),
		('PN4','SP01','200','10000'),
...
VALUES	('hd1','SP01','400','5000','KM1'),
		('hd2','SP04','20','500000','KM2'),
		('hd3','SP03','11','7990000','KM3'),
		('hd4','SP04','21','50000','KM1'),
		('hd5','SP01','23','102000','KM3'),
		('hd6','SP04','10','6000','KM5'),
		('hd7','SP02','50','500000','KM7'),
		('hd8','SP03','100','560000','KM8'),
		('hd9','SP04','1','90000','KM8'),
		('hd10','SP04','10','100000','KM9');
...
where maSP not in ( select maSP from ChiTietDonHang )
	...
	-- thay where bang join
	select SanPham.maSP,tenSP
	from SanPham
		left join ChiTietDonHang as C
		on sanpham.maSP =c.maSP
	where maDH is null
...
where trangThaiDonHang = N'Đã giao' and DDH_HD.maNV = NV.maNV
	...
	-- thay where cho join
	select maDH,NV.maNV,tenNV,Email,SDT
	from DonDatHang_HoaDon as DDH_HD
		join NhanVien as NV
		on DDH_HD.maNV =  NV.maNV
...
from KhachHang as KH  
	join DonDatHang_HoaDon as DDH_HD
	on KH.maKH = DDH_HD.maKH
	join XaPhuong as XP
	on	DDH_HD.idXP = xp.idXP
	join QuanHuyen as QH
	on  XP.idQH= QH.idQH
	join TinhThanh as TT
	on TT.idTT =QH.idTT
...
select a.maSP as N'Mã sản phẩm', b.tenSP as N'Tên sản phẩm', 
	format(avg(distinct a.donGia),'##,#\ VNĐ','es-ES') as N'Giá bán trung bình', 
	format(max(a.donGia),'##,#\ VNĐ','es-ES') as N'Giá bán max',
	format(min(a.donGia),'##,#\ VNĐ','es-ES') as N'Giá bán min'
...
having count(PN.maNCC) >3
	...
	-- cách join khác
	select p.maNCC, n.tenNCC, n.diaChiNCC, n.SDT, n.nhanVienLienHe, count (p.maNCC) as N'Ső lần nhập'
	from NhaCungCap as n
		right join PhieuNhap as p   -- lấy dl từ bản phiếu nhập
		on n.maNCC = p.maNCC
	group by p.maNcc, n.tenNCC, n.diachiNCC,  n.SDT, n.nhanvienLienHe
	having count(p.maNCC) > 3


-- 2. Hiển thị những đơn hàng không cần được xuất hóa đơn
select * 
from DonDatHang_HoaDon 
...
from DonDatHang_HoaDon as DDH
	join ChiTietDonHang as ctdh
	on ctdh.maDH=DDH.maDH and DDH.trangThaiDonHang in (N'Đã duyệt – Đang đóng gói', N'Đang giao', N'Chờ duyệt')

--3c. tuong tu cho bang phieu nhap
go
create view NhungDonChuaNhap 
as
	select maPN, maNCC
	from PhieuNhap
...
SELECT *,
     CASE
		   ...
		   WHEN soluongHienCon = 20 THEN N'Số lượng bằng 20'
		   ELSE N'Số lượng nhỏ hơn 20'
                        
END AS Result
FROM SanPham;

-- hay hien thi masp , ten sp, so luong da ban, if >0 da ban else chua ban

select s.maSP, tenSP, sum(soLuongDat) as SLD,
		 case
			when sum(soluongdat)>0 then N'Đã bán'
			else N'Chưa bán được cái nào'
		 end as result
from SanPham as s
    left join ChiTietDonHang as c
    on s. maSP = c.maSP
...
-- lỗi ở then
--Hãy viết đoạn lệnh để kiểm tra: nếu tồn tại một sản phẩm có số lượng hiện còn nhở hơn 0 -> 'Không đủ sản phẩm để bán - hãy chọn lại đơn hàng'; ngược lại 'Bạn đã đặt hàng thành công'
select s.maSP, tenSP, sum(soLuongDat) as SLD,
		 case
			when soluongHienCon <0 then N'Không đủ sản phẩm để bán - hãy chọn lịa đơn hàng'
		else N'Chưa bán được cái nào'
		 end as result
from SanPham as s
    left join ChiTietDonHang as c
...
group by s.maSP, tenSP
 ...
 -- exists : tồn tại
begin 
	if exists ( select* from SanPham where soluongHienCon <0)
	print N'Không đủ sản phẩm để bán, hãy chọn lại!'
	else
	print N'Bạn đã đặt hàng thành công!'
end

/*
update SanPham
set soluongHienCon = -10
where maSP = 'sp0001' */
...
--viết đoạn lệnh để lấy giá trị lớn nhất của id trong bảng khách hàng
	-- maKH là int vd: maKH : 1
	begin
		declare @maxIDKH int
		set @maxIDKH = (select max(maKH) from KhachHang)
		print @maxIDKH
	end

	-- maKH là text vd: mahk :KH0001
	begin
		declare @idmax int
		set @idmax= (select max (right (maKH, 5))
		from KhachHang)
		print @idmax
		--có khnext có stt là 6 - nối KH phía trước(để đề xuất maKh thêm mới)
		declare @khnext char(7)
		set @khnext = CONCAT ('KH', format (@idmax+1, 'D5'))
		print @khnext
	end

-- concat hàm nối
-- print format(6, 'D5'): hàm bổ sung số 0000 vd:000003
-- cách khác, cách declare ở trên
select maKH
from (select max(right(makh, 5)) as RMKH
      from KhachHang) as tmp,
      KhachHang as k
where right(k.maKH, 5)=RMKH


-- hãy hiển thị thông tin các đơn hàng: nếu tổng số tiền mà lớn hơn 1tr thì hiển thị ' Đước giảm 10%'
-- nếu tổng tiền từ 400k trở lên thì được miễn phí ship
-- còn lại: 'Ship tùy theo trọng lượng và khoảng cách'
...
begin
	...
	set @idmax= (select max (right (maKH, 5))
				from KhachHang)
	print @idmax
	if exists ( select d.maDH
				from ChiTietDonHang as c, DonDatHang_HoaDon as d
				where soLuongDat*donGia > 1000000 and c.maDH = d.maDH
				group by d.maDH
	)
...
select c.maDH, SUM(soLuongDat* donGia) as tongSoTien,
     case
		 when SUM(soLuongDat *donGia)> 10000000 then N'Được giảm 18%'
		 when SUM(soLuongDat*donGia) >= 400000 then N'Miễn phí ship'
     else
		  N'Ship tùy theo trọng lượng và khoảng cách'
     end as N'Khuyến mãi'
